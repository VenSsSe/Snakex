...
function drawScene(targetCtx, width, height, currentSnake, currentHeadScale, currentHeadOpacity, isRecording = false, currentBerryPositions = berryPositions) {
    ...
    const applyPillowEffect = (ctxToApply, cellX, cellY, intensity) => {
    ...
    const applyGlintEffect = (ctxToApply, cellX, cellY, intensity) => {
        const alpha = intensity;
        if (alpha === 0) return;
    ...
    const applyVignetteEffect = (ctxToApply, cellX, cellY, intensity) => {
        const alpha = intensity;
        if (alpha === 0) return;
        const centerX = cellX + effectiveCellSize / 2;
        const centerY = cellY + effectiveCellSize / 2;
    ...
    const applyConvexityEffect = (ctxToApply, cellX, cellY, style, intensity) => {
        switch (style) {
            case 'glint': applyGlintEffect(ctxToApply, cellX, cellY, intensity); break;
            case 'vignette': applyVignetteEffect(ctxToApply, cellX, cellY, intensity); break;
            default: applyPillowEffect(ctxToApply, cellX, cellY, intensity); break;
        }
    };
            ...
            for (let y = 0; y < GRID_SIZE; y++) {
                for (let x = 0; x < GRID_SIZE; x++) {
                    ...
                    if (isVolumeEffectEnabledBg) {
                        applyConvexityEffect(targetCtx, cellX, cellY, volumeEffectStyleBg, volumeEffectIntensityBg);
                    }
                }
            }
        }
    ...
    if (!isGameEnd && (!isGameOver || isRecording)) {
        ...
        if (coverImage) {
            ...
            const isVideo = source.tagName === 'VIDEO';
            const isReady = isVideo ? source.readyState >= 2 : source.complete;
            if (isReady && (isVideo ? source.videoWidth : source.width) > 0) {
        ...
        if (isVolumeEffectEnabledCover) {
            ...
            for (let y = 0; y < GRID_SIZE; y++) {
                for (let x = 0; x < GRID_SIZE; x++) {
                    applyConvexityEffect(maskCtx, x * effectiveCellSize, y * effectiveCellSize, volumeEffectStyleCover, volumeEffectIntensityCover);
                }
            }
        }
