...
fadeSlider.addEventListener('input', updateFadeDuration);
...
// Listen for a long press (hold) to clear everything
resetResourcesBtn.addEventListener('mousedown', () => {
    if (resetResourcesBtn.disabled) return;
    
    holdTimeout = setTimeout(() => {
        resetResourcesBtn.disabled = true;
        clearAllResources();
        showModal('Все ресурсы стерты.');
        
        setTimeout(() => {
            resetResourcesBtn.disabled = false;
        }, 2000);

    }, 400);
});

resetResourcesBtn.addEventListener('mouseup', () => clearTimeout(holdTimeout));
resetResourcesBtn.addEventListener('mouseleave', () => clearTimeout(holdTimeout));