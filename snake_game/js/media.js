    ...
    return new Promise((resolve, reject) => {
        ...
        img.onload = () => {
            ...
            switch(type) {
                ...
                case 'berry': berryImage = img; break; // Don't reset game here, let resetAllResources do it
            }
            saveSettings();
            resolve();
        };
...
async function resetAllResources() {
    const resourceTypes = ['image', 'cover', 'snake-head', 'berry'];

    resourceTypes.forEach(type => {
        const oldPreview = document.getElementById(`preview-${type}`);
        if (oldPreview) {
            const mediaElement = oldPreview.parentElement.dataset.mediaElement;
    ...
    const defaultImagePaths = [
        { url: 'images/default_background.svg', type: 'image' },
        { url: 'images/default_cover.svg', type: 'cover' },
        { url: 'images/default_snake_head.svg', type: 'snake-head' },
        { url: 'images/default_berry.svg', type: 'berry' }
    ];

    try {
        await Promise.all(defaultImagePaths.map(p => handleImageURL(p.url, p.type)));
        showModal('Все ресурсы сброшены до стандартных.');
    } catch (error) {
        showModal('Ошибка при загрузке стандартных ресурсов.');
    } finally {
        resetGame();
    }
}
...
function clearAllResources() {
    ...
    // Set all image variables to null
    backgroundImage = null;
    coverImage = null;
    snakeHeadImage = null;
    berryImage = null;
    ...
    // Reset the game to show the blank state
    resetGame();
}